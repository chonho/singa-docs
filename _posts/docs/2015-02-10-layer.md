...
There are some common fields for all kinds of layers:
  ...
  * `type`: an integer used for identifying a Layer subclass. The types of built-in
  ...
  The detail is explained in a section [below](#newlayer).
...
The param local_size has its specific use when choose the different norm_region. For WITHIN_CHANNEL, it means the side length of the space region which will be summed up. For ACROSS_CHANNELS, it means the quantity of the adjoining channels which will be summed up
